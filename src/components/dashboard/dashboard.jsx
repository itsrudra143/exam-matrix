  ...
  useEffect(() => {
    if (testAttempts && testAttempts.length > 0) {
      ...
      const subjectData = Object.keys(subjectMap).map(title => {
        ...
        else if (title.toLowerCase().includes('web') || title.toLowerCase().includes('stack')) icon = '🌐';
        else if (title.toLowerCase().includes('system') || title.toLowerCase().includes('os')) icon = '⚙️';
